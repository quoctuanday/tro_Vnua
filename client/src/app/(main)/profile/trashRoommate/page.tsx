...
function TrashRoommatePage() {
    const [roommates, setRoommates] = useState<Roommate[]>([]);
    const [roomSelected, setRoomSelected] = useState<Roommate | null>(null);
    ...
    useEffect(() => {
        const getData = async () => {
            const response = await getDeleteRoommatePersonal();
            if (response) {
                console.log(response.data.rooms);
                setRoommates(response.data.rooms);
            }
        };
        getData();
    }, []);
    ...
    const handelRestore = (roomId: string) => {
        const restore = async () => {
            ...
            if (response) {
                ...
                setRoommates(roommates.filter((room) => room._id !== roomId));
            } else {
...
export default TrashRoommatePage;
